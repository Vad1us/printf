...
static void	ft_hex(char c, t_style *style, unsigned long long i)
{
	char	*s;
...
{
	if (style->l == 1)
		ft_hex(c, style, va_arg(vlist, unsigned long));
	else if (style->h == 1)
		ft_hex(c, style, va_arg(vlist, unsigned int));
	else if (style->ll == 1)
		ft_hex(c, style, va_arg(vlist, unsigned long long));
	else if (style->hh == 1)
		ft_hex(c, style, va_arg(vlist, unsigned int));
	else if (style->j == 1)
		ft_hex(c, style, va_arg(vlist, uintmax_t));
	else if (style->z == 1)
		ft_hex(c, style, va_arg(vlist, size_t));
	else
		ft_hex(c, style, va_arg(vlist, unsigned int));
}
