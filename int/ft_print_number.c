...
static void	ft_number(t_style *style, long long i)
{
	if (style->h == 1)
		i = (short)i;
	if (style->hh == 1)
...
void		ft_print_number(char c, t_style *style, va_list vlist)
{
	if (c == 'D')
	{
		...
		style->ll = 0;
		style->j = 0;
		ft_number(style, va_arg(vlist, long long));
	}
	else if (style->l == 1)
		ft_number(style, va_arg(vlist, long));
	else if (style->h == 1)
		ft_number(style, va_arg(vlist, int));
	else if (style->ll == 1)
		ft_number(style, va_arg(vlist, long long));
	else if (style->hh == 1)
		ft_number(style, va_arg(vlist, int));
	else if (style->j == 1)
		ft_number(style, va_arg(vlist, intmax_t));
	else if (style->z == 1)
		ft_number(style, va_arg(vlist, size_t));
	else
		ft_number(style, va_arg(vlist, int));
}
